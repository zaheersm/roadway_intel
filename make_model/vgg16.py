...
NO_CLASSES = 841
NO_CLASSES_IMAGENET = 1000

class vgg16:
    def __init__(self):
        pass

    def inference_imagenet (self,images):
        self.images = images
        self.convlayers()
        self.fc_layers(NO_CLASSES_IMAGENET)
        # Logits (softmax_linear)
        logits = self.fc3l  
        self.probs = tf.nn.softmax(logits)
        return logits
    
    def inference_vehicle(self, images):
        self.images = images
        self.convlayers()
        self.fc_layers(NO_CLASSES)
        logits = self.fc3l
        self.probs = tf.nn.softmax(logits)
        return logits
    ...
    def train(self, loss, learning_rate, optimizer='sgd'):
        """
    ...
    def fc_layers(self, output_units):
        # fc1
        ...
        with tf.name_scope('fc3') as scope:
            fc3w = tf.Variable(tf.truncated_normal([4096, output_units],
                                                         dtype=tf.float32,
                                                         stddev=1e-1), name='weights')
            fc3b = tf.Variable(tf.constant(1.0, shape=[output_units], dtype=tf.float32),
                                 trainable=True, name='biases')
...
if __name__ == '__main__':
    ...
    images, labels = input.inputs(True, 5, 1)
    vgg = vgg16()
    logits = vgg.inference_vehicle(images)
    loss = vgg.loss(logits, labels)
    train_op = vgg.train(loss, 0.0001, 'adam')
